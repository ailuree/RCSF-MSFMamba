...
def _select_test_index(dataset, index_mat, data_info, eval_split):
    default_test_key = data_info['keys'][4]
    selected_key = default_test_key
    selected_split = 'full'

    if dataset == 'Houston2018':
        requested_key = 'houston_test_small' if eval_split == 'small' else default_test_key
        if requested_key in index_mat:
            selected_key = requested_key
            selected_split = 'small' if requested_key == 'houston_test_small' else 'full'
        else:
            print(f"Houston2018 eval split '{eval_split}' unavailable, fallback to full test set.")

    test_index = index_mat[selected_key]
    return test_index, selected_split, selected_key
...
def get_loader(dataset, batchsize, num_workers=0, useval=0, pin_memory=True, eval_split='small'):
    ...
    train_index = index_mat[data_info['keys'][3]]
    test_index, selected_split, selected_key = _select_test_index(dataset, index_mat, data_info, eval_split)
    trntst_index = np.concatenate((train_index, test_index), axis=0)
    all_index = index_mat[data_info['keys'][5]]
    ...
    recommended_workers = min(4, os.cpu_count() or 0)
    print('num_workers=' + str(num_workers))
    if recommended_workers > 0 and num_workers == 0:
        print(f"Suggestion: try --num_work 2 or --num_work {recommended_workers} for faster loading.")
    print(f"Eval split: {selected_split} ({selected_key}), test samples: {test_index.shape[0]}, test batches: {len(test_loader)}")
